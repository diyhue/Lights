...
//settings
char lightName[LIGHT_NAME_MAX_LENGTH] = "Hue Gradient Lightstrip";
...
void processLightdata(uint8_t light, float transitiontime = 4) { // calculate the step level of every RGB channel for a smooth transition in requested transition time
  transitiontime *= 14 - (pixelCount / 40); //every extra led add a small delay that need to be counted for transition time match
