...
bool saveConfig() {
  ...
  for (uint16_t i = 0; i < lightsCount; i++) {
    json["lightSplit_" + String(i)] = lightSplitsArray[i];
  }  
...
bool loadConfig() {
  ...
  for (uint16_t i = 0; i < lightsCount; i++) {
    lightSplitsArray[i] = (uint16_t) json["lightSplit_" + String(i)];
  }
...
void setup() {
  ...
  server.on("/config", []() {
    ...
    for (uint8_t i = 0; i < lightsCount; i++) {
      root["lightSplit_" + String(i)] = (int)lightSplitsArray[i];
    }
  ...
  server.on("/", []() {
    if (server.arg("section").toInt() == 1) {
      ...
      for (uint16_t i = 0; i < lightsCount; i++) {
        lightSplitsArray[i] = server.arg("lightSplit_" + String(i)).toInt();
      }
