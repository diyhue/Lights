...
void lightEngine() {
  ...
  if (inTransition) {
    //wthis will be executed when one time when light is off in order to avoid the flash generated by flash write
    if (!lightState && startup == 0 && currentColors[0] == 0 && currentColors[1] == 0 && currentColors[2] == 0 && currentColors[3] == 0 && currentColors[4] == 0) {
      saveState();
    }
    delay(6);
    inTransition = false;
...
void setup() {
  ...
  //Serial.println("#1 Lightstate is " + (String) lightState);
  ...
  apply_scene(scene);
  stepLevel[0] = colors[0] / 150.0; stepLevel[1] = colors[1] / 150.0; stepLevel[2] = colors[2] / 150.0; stepLevel[3] = colors[3] / 150.0; stepLevel[4] = colors[4] / 150.0;
